...
function get_scratch () {
    if [[ -z "$SCRATCH" ]]; then
        SCRATCH=$(mktemp -d ibhtml2img-XXXXXX)
    fi
}
...
function do_target_xulapp () {
    ...
    if [ -n "$IBHTML2IMG_APP_NAME" ]; then
        sed -i -e "s/Name=ibhtml2img/Name=${IBHTML2IMG_APP_NAME}/" "${SCRATCH}/application.ini"
    fi
    ...
    ##
    ## end preprocessing
    zip -r ../ibhtml2img.xulapp * > /dev/null
    popd > /dev/null
    do_cleanup
    echo ok
}
...
function do_target_dist_tar () {
    do_target_xulapp
    get_scratch
    ## now we have ibhtml2img.xulapp
    ## package it with install.sh
    ##
    ## some other files should probably go in here.. NEWS, for example
    mkdir "$SCRATCH/ibhtml2img-$VERSION"
    mv ibhtml2img.xulapp "$SCRATCH/conkeror-$VERSION/"
    cp install.sh "$SCRATCH/ibhtml2img-$VERSION/"
    pushd "$SCRATCH" > /dev/null
    tar c ibhtml2img-$VERSION | gzip > ibhtml2img-$VERSION.tar.gz
    popd > /dev/null
    mv "$SCRATCH/ibhtml2img-$VERSION.tar.gz" .
    echo -n "Making ibhtml2img-$VERSION.tar.gz ..."
    do_cleanup
    echo ok
}
...
function do_target_release () {
    ...
    ##
    do_target_dist_tar
    echo -n Putting ibhtml2img-$VERSION.tar.gz in downloads directory ...
    mv ibhtml2img-$VERSION.tar.gz ../downloads
    echo ok
}
...
function do_target_help () {
    ...
    echo "be \`<IBHTML2IMG>/src' where <IBHTML2IMG> is the project root."
    echo "This script expects to find the subdirectory structure,"
    echo "\`ibhtml2img/content', and VERSION in the current directory,"
    ...
    echo "                        \`ibhtml2img/content/*.js'  Modifies no files."
    echo
